...
User preferences live in the app's config (per-machine `userData/config.json`), **not** in the vault — they're not note data and don't travel with an exported vault. Today: `theme`, `reduceTransparency`, and `embedding` — the semantic-search provider config `{ provider: 'off' | 'openai-compatible', baseUrl, model, apiKey? }`, default `off` (keyword only, no network). The owner sets it in Settings → Search & indexing; the main process builds an embedding provider from it ([ADR 0007](../adr/0007-embeddings-provider-config-and-vector-storage.md)). The API key is a credential — it stays in local app config and is never written into the vault.
