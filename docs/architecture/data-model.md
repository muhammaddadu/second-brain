...
User preferences live in the app's config (per-machine `userData/config.json`), **not** in the vault — they're not note data and don't travel with an exported vault. Today: `theme`, `reduceTransparency`, and `embedding` — the semantic-search config `{ enabled, kind, configs }` where `kind` is the chosen provider (`ollama | lmstudio | openai | openai-compatible | bedrock`) and `configs` holds each provider's non-secret fields (base URL / model / region). Default is `enabled: false` (keyword only, no network). Providers are **adapters behind one interface** ([ADR 0008](../adr/0008-embedding-provider-adapters-and-discovery.md)); the owner picks and configures one in Settings → Semantic search, with local runtimes discovered by a "Scan this machine" probe. **Secrets** (API keys, AWS credentials) are **never** stored here in plaintext or in the vault — they're encrypted with the OS keychain (Electron `safeStorage`) and kept in a separate `secrets` map in the app config as ciphertext, resolved only in the main process.
