...
**Status: partly adopted** — choices below were proposed at docs time (2026-07-07); those adopted by shipped epics (pnpm/Vitest/Biome/TS in E0, Electron/React/Tailwind/Playwright in E1, BlockNote in E2, chokidar in E3, Mermaid in E7, WASM SQLite index + OpenAI-compatible embeddings in E4) are now in use, all adopted through E6. Record reversals here with a dated note, don't erase the original rationale.
...
| **MCP TypeScript SDK** (stdio) | agent surface | official SDK (`@modelcontextprotocol/sdk` + zod schemas); stdio transport is what Claude Code / desktop clients spawn. Adopted E6: a data-driven tool registry over core | E6 ✓ |
