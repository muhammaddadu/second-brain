...
**Status: partial** — E0 scaffolded the workspace and `packages/core`; E1 added `apps/desktop` (Electron shell + IPC bridge); E2 added the BlockNote editor with autosave, tag editing, and core's Markdown import/export. The CLI/MCP packages and search are still planned. Keep this doc in lockstep as packages land.
