...
**Concurrency (operator note):** headless `update_note` replaces the note body directly — it does **not** use the editor's compare-and-swap guard (an agent has no `baseHash` to compare). Writes are still atomic (no torn files, [ADR 0002](../adr/0002-vault-concurrency-atomic-write-rename.md)), but if the desktop app and an agent edit the *same note* at the same instant, the later write wins for that note (the app side surfaces this via its live watcher + conflict banner; the agent side does not). In practice agents and the human rarely edit the identical note simultaneously; if you script bulk agent edits while actively editing in the app, expect last-writer-wins per note. Read-modify-write on the freshest content to minimise the window.
