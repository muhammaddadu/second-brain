...
- **Drop files to import**: drag `.md`/`.txt`/`.docx`/`.pdf`/`.csv`/`.xlsx` onto the tree (or a folder). Documents become notes. **Spreadsheets (CSV/XLSX) prompt to import as a Database** (a folder of rows you can sort/filter/board, with column types inferred) **or a Note** (a table) — the app recommends based on size (small table → database, large dump → note); multiple worksheets become a folder of tables. Imports run off the UI thread with an inline **progress** toast (row counts for big files); titles come from the filename, nothing is overwritten, and unsupported/corrupt files report a per-file reason.
