...
> **TODO — no code is scaffolded yet.** There is no `package.json`; do not invent commands. When the monorepo lands in epic E0, record the real `dev` / `lint` / `typecheck` / `test` / `build` commands here **in the same change**, copied verbatim from the workspace scripts.

**Before declaring any change done:** once E0 lands, the lint + typecheck + test + build line recorded above must pass. Until then: the docs cross-links in `docs/README.md` must resolve.

## Repo Layout

> **Planned** (traces to [app-architecture](docs/architecture/app-architecture.md)); today the repo contains only docs.
...
packages/core/       # (planned, E0) vault library: files, note envelope/metadata, tree, index — ALL vault I/O lives here
