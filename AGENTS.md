...
**Stack:** TypeScript throughout. Electron + React + Vite for the desktop app; BlockNote for editing; a shared core library for all vault operations; WASM SQLite (FTS5 + vector embeddings) for the derived search index; a CLI and an MCP server as headless agent surfaces. **Shipped so far:** E0 (core), E1 (desktop shell), E2 (editor), E7 (Mermaid), E3 (file actions + watcher + conflict guard), E4 (keyword + semantic hybrid search, multi-provider embeddings, knowledge graph), E5 (`brain` CLI), E6 (`brain-mcp` MCP server + seeded vault rules), E8 (databases: folder + schema, rows as notes, table/board views), E9 (wikilinks: `[[note]]` links, backlinks, graph edges), E11 (multi-hop recall over the graph — CLI / MCP / Related panel). **All planned v1 epics have shipped**, and the desktop app is packaged for distribution (electron-builder, per-OS installers — [ADR 0011](docs/adr/0011-packaging-and-distribution-electron-builder.md); build steps in [building-and-releasing](docs/guides/building-and-releasing.md)). Proposed post-v1 ideas live in [E10](docs/product/epics/E10-deeper-memory-semantics.md) (not scheduled; multi-hop graduated to E11). Marketing site: `apps/web` (`pnpm dev:web`). See [tech-stack](docs/architecture/tech-stack.md) for rationale and alternatives considered.
...
Traces to [app-architecture](docs/architecture/app-architecture.md). `packages/core` (E0), `apps/desktop` (E1–E4, E7), `packages/cli` (E5), and `packages/mcp` (E6) have all landed.
